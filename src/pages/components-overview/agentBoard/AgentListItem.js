...
const AgentListItem = memo(({ agent }) => {
  ...
  const handleUpdate = () => {
    setSelectedAgentId(agent_id); // Set the selected agent_id when clicked
    setOpen(true);
  };
  return (
    <Grid item xs={3}>
      <Box>
        <Box>
          <Box sx={{ boxShadow: '0px 4px 20px rgba(0, 0, 0, 0.1)' }}>
            <Box>
              <Typography sx={{ backgroundColor: '#008000', color: '#fff', px: '10px', py: '0px' }}>{agent_status}</Typography>
            </Box>
      ...
      <UpdateAgent open={open} handleClose={handleClose} selectedAgentId={selectedAgentId}></UpdateAgent>
    </Grid>
  );
});
